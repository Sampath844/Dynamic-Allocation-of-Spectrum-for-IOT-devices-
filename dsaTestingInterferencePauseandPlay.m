...
num_channels = 6;               % Number of channels available
num_time_steps = 10;             % Number of time steps for data transmission
...
% Dynamic Graph: Plot the optimized channel assignments over time
figure;
hold on;

% Loop through each time step and plot each device's channel assignment
for t = 1:num_time_steps
    % Loop through each device and plot its channel usage up to the current time step
    for dev = 1:num_devices
        % Get the optimized channel assignments for this device up to the current time step
        device_channels = optimized_channels(dev, 1:t);
        
        % Plot the device's channel usage over time (y-axis = channel, x-axis = time)
        plot(1:t, device_channels, '-o', 'MarkerFaceColor', 'b', 'MarkerEdgeColor', 'k', 'LineWidth', 2);
        
        % Annotate with device number and priority
        text(t, device_channels(end), sprintf('Dev %d (P%d)', dev, device_priorities(dev)), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left', 'FontSize', 8);
    end
    
    % Title and labels
    title(sprintf('Optimized Channel Assignments (Time Step %d)', t));
    xlabel('Time');
    ...
    drawnow;
    pause(2); % Adjust for animation speed, increase or decrease as needed
end
