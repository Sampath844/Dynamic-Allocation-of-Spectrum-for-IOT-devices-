...
% Graph: Plot the optimized channel assignments over time with additional information
figure;
hold on;
...
for dev = 1:num_devices
    ...
    % Annotate each point with device number, priority, and RFID data
    for t = 1:num_time_steps
        x = t;  % time step on x-axis
        y = device_channels(t);  % channel assignment on y-axis
        label = sprintf('D%d (P:%d, Data:%d)', dev, device_priorities(dev), rfid_data(dev, t));
        text(x, y, label, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right', 'FontSize', 8);
    end
end

title('Optimized Channel Assignments for Devices with Priority and Data');
xlabel('Time');
